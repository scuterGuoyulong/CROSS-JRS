...
def main():
    ...
    lr = 0.0001
    weights = [0.01, 1, 1, 1]  # loss weights
    save_dir = 'CROSS_JRS_mse_{}_weakly_{}_diffusion_{}_seg_{}_lr_{}_sameseed_100epoch/'.format(weights[0], weights[1],weights[2],weights[3],lr)
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        loss_seg_x=utils.AverageMeter()
        loss_seg_y=utils.AverageMeter()
        idx = 0
        for data in train_loader:
            ...
            for n, loss_function in enumerate(criterions):
                ...
                loss += curr_loss
            # seg loss
            seglossx=segloss(output[2].float(),x_seg.float())*weights[3]
            loss+=seglossx
            seglossy=segloss(output[3].float(),y_seg.float())*weights[3]
            loss+=seglossy
